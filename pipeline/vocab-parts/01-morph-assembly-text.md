...
| `fillToStroke` | **built: FillToStroke** | `dur_f=24, ease=expoInOut` | Identical geometry crossfades solid-fill→outline-stroke via two stacked SVG groups (`fill-opacity 1→0` / `stroke-opacity 0→1`, same path). Reads as "dematerialising." | `B_logo-wip`; MOVE_VOCAB_ADDITIONS `fillToStrokeCrossfade` |
